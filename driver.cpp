...
{
	tTrackSeg* seg = car->_trkPos.seg;
	if (car->_speed_x < MAX_UNSTUCK_SPEED || pit->getInPit()) return accel;
...
/* Filter for the brake value, responsible for the pit speed limit and stop */
...
/* Computes the brake distance from the current speed to the allowed speed */
